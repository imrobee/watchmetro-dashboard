...
# # Process vehicles for pie chart
...
#     # 2nd row: Pie Chart and Monthly Accidents
#     html.Div([
#         html.Div([dcc.Graph(id='vehicle-pie-chart')], className='card graph-card'),
...
# # Callback for immediate time button visual feedback
# @app.callback(
...
#     return ['button active' if id['index'] == time_filter else 'button' for id in time_ids]

# # Callback for initial dashboard load with default filters
# @app.callback(
#     Output('vehicle-pie-chart', 'figure', allow_duplicate=True),
...
#     pie_chart = px.pie(
#         values=vehicle_counts_filtered.values,
#         names=vehicle_counts_filtered.index,
...
#     return pie_chart, monthly_chart, hourly_chart, map_html, loading_style
...
#     Output('loading-overlay', 'style', allow_duplicate=True),
#     Input('apply-filters-btn', 'n_clicks'),
#     State({'type': 'time-btn', 'index': ALL}, 'n_clicks'),
...
#     prevent_initial_call=True
# )
# def update_dashboard_content(apply_clicks, time_clicks, selected_month, selected_city, theme_value, time_ids):
#     # Determine active time filter
#     time_filter = 'All Day'  # default
#     max_clicks = max(time_clicks) if time_clicks else 0
#     if max_clicks > 0:
#         max_index = time_clicks.index(max_clicks)
...
#     # 1. Vehicle Pie Chart (filtered by time and month)
#     vehicles_series = filtered_df['Involved'].str.split(r"\s+AND\s+|\s*,\s*").explode()
#     vehicles_series = vehicles_series.apply(clean_vehicle_name)
#     vehicles_series = vehicles_series.dropna()
#     vehicle_counts = vehicles_series.value_counts()
...
#     # 3. Hourly Accidents (filtered by time and month)
#     hourly_counts = filtered_df['Hour'].dropna().value_counts().sort_index()
...
#     # Button styling
#     time_classes = ['button active' if id['index'] == time_filter else 'button' for id in time_ids]
...
#             .toggle-checklist input:checked + label {
...
#              # move the knob when the input inside this label is checked
#             .toggle-checklist label:has(input:checked)::before {
#               transform: translateX(26px);
#               }

#              # dark track color when checked
...
#                 color: white !important;
#             }
#             .dark-theme .toggle-checklist input:checked + label:before {
